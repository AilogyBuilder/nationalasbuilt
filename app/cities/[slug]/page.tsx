...
export default function CityPage({ params }: Props) {
  ...
  return (
    <>
      {/* ── HERO ── */}
      <section style={{ background: "linear-gradient(135deg, var(--navy) 0%, #1a3568 55%, #1e4d9b 100%)", color: "white", padding: "80px 0 64px", position: "relative", overflow: "hidden" }}>
        ...
        <div className="container" style={{ position: "relative" }}>
          ...
          {/* Breadcrumb */}
          <nav aria-label="breadcrumb" style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "rgba(255,255,255,0.45)", marginBottom: 28 }}>
            <Link href="/" style={{ color: "rgba(255,255,255,0.45)", textDecoration: "none" }}>Home</Link>
            <span>/</span>
            <span>Locations</span>
            <span>/</span>
            <span style={{ color: "var(--silver-light, #c5d0df)" }}>{city.name}, {city.state}</span>
          </nav>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 400px", gap: 56, alignItems: "center" }}>
            {/* Left */}
            <div>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", color: "#c5d0df", fontSize: 12, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", padding: "6px 14px", borderRadius: 100, marginBottom: 24 }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#3b7de8", display: "inline-block" }} />
                {city.metro}
              </div>
              <h1 style={{ fontFamily: "var(--font-display, Georgia, serif)", fontSize: "clamp(32px, 4vw, 52px)", fontWeight: 600, lineHeight: 1.1, letterSpacing: "-0.02em", marginBottom: 20 }}>
                {city.heroLine}
              </h1>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
                ...
                <a href={COMPANY.phoneHref} className="btn btn-outline-white btn-lg">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 10.8a19.79 19.79 0 01-3.07-8.67A2 2 0 012.18 0h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 7.91a16 16 0 006.29 6.29l1.28-1.28a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/>
                  </svg>
                  {COMPANY.phone}
                </a>
              </div>
            ...
            <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10, padding: 32 }}>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(255,255,255,0.35)", marginBottom: 16 }}>Key areas we serve</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 28 }}>
                {city.keyAreas.map((area) => (
                  <span key={area} style={{ background: "rgba(30,77,155,0.3)", border: "1px solid rgba(45,95,179,0.3)", color: "rgba(255,255,255,0.8)", fontSize: 13, fontWeight: 500, padding: "5px 12px", borderRadius: 100 }}>
                    {area}
                  </span>
                ))}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                {[
                  ...
                  <div key={s.num} style={{ background: "rgba(255,255,255,0.04)", borderRadius: 6, padding: "14px 16px" }}>
                    <div style={{ fontFamily: "var(--font-display, Georgia, serif)", fontSize: 20, color: "#c5d0df", lineHeight: 1 }}>{s.num}</div>
      ...
      <div style={{ background: "var(--bg)", borderBottom: "1px solid var(--border)", padding: "16px 0" }}>
        <div className="container">
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 40, flexWrap: "wrap" }}>
            {["Founded 2004 — 20+ years", "LEICA ±1/32\" precision", "DWG + PDF deliverables", "All 50 states + PR & Canada"].map((t) => (
      ...
      <section className="section">
        <div className="container">
          <div style={{ marginBottom: 48 }}>
            ...
            <h2 style={{ fontFamily: "var(--font-display, Georgia, serif)", fontSize: "clamp(26px, 3vw, 40px)", color: "var(--navy)", marginTop: 8 }}>
              What We Deliver in{" "}
              <em style={{ color: "var(--navy-mid)" }}>{city.name}, {city.state}</em>
            </h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }}>
            {SERVICES.map((s) => (
              <div key={s.id} style={{ background: "white", border: "1px solid var(--border)", borderRadius: 8, padding: "24px 22px" }}>
                <h3 style={{ fontFamily: "var(--font-display, Georgia, serif)", fontSize: 17, color: "var(--navy)", marginBottom: 8 }}>{s.title}</h3>
                <p style={{ fontSize: 13, lineHeight: 1.65, color: "var(--text-mid)", marginBottom: 14 }}>{s.desc}</p>
                <a href="#quote" style={{ fontSize: 13, fontWeight: 600, color: "var(--blue)", textDecoration: "none" }}>
                  Quote in {city.name} →
                </a>
              </div>
            ))}
      ...
      <section className="section" style={{ background: "var(--bg)" }}>
        <div className="container">
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            ...
            <h2 style={{ fontFamily: "var(--font-display, Georgia, serif)", fontSize: "clamp(26px, 3vw, 38px)", color: "var(--navy)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 0, position: "relative" }}>
            ...
            {PROCESS.map((step) => (
              <div key={step.num} style={{ textAlign: "center", padding: "0 28px" }}>
                <div style={{ width: 68, height: 68, borderRadius: "50%", background: "var(--navy)", color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-display, Georgia, serif)", fontSize: 24, margin: "0 auto 20px", position: "relative", zIndex: 1 }}>
                  {step.num}
                </div>
                <h3 style={{ fontFamily: "var(--font-display, Georgia, serif)", fontSize: 18, color: "var(--navy)", marginBottom: 10 }}>{step.title}</h3>
                <p style={{ fontSize: 14, lineHeight: 1.65, color: "var(--text-mid)" }}>{step.desc}</p>
              </div>
            ))}
          </div>
          <style>{`@media(max-width:700px){section[style*="var(--bg)"] .container>div:last-child{grid-template-columns:1fr!important;gap:36px!important}}`}</style>
        </div>
      </section>
        ...
        <div className="container">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "start" }}>
            <div>
              ...
              <h2 style={{ fontFamily: "var(--font-display, Georgia, serif)", fontSize: "clamp(26px, 3vw, 38px)", color: "var(--navy)", marginTop: 12, marginBottom: 18 }}>
                The Right Partner for{" "}
                <em style={{ color: "var(--navy-mid)" }}>{city.name} Projects</em>
              </h2>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[
                { title: `${city.name} market coverage`, body: `We serve ${city.keyAreas.join(", ")} and the broader ${city.metro} area with fast local scheduling.` },
                { title: "48-hour scheduling", body: "Survey teams scheduled within 48 hours of your request — no waiting on distant crews." },
                ...
                <div key={item.title} style={{ display: "flex", gap: 16, padding: "18px 20px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 8 }}>
          ...
          <style>{`@media(max-width:800px){section .container>div[style*="1fr 1fr"]{grid-template-columns:1fr!important}}`}</style>
        </div>
      </section>
      ...
      <section className="section" id="quote" style={{ background: "var(--navy)" }}>
        <div className="container">
          <div style={{ marginBottom: 40 }}>
            <span className="eyebrow" style={{ color: "#3b7de8" }}>Get started in {city.name}</span>
            <h2 style={{ fontFamily: "var(--font-display, Georgia, serif)", fontSize: "clamp(26px, 3vw, 38px)", color: "white", marginTop: 12 }}>
              Request a Quote for{" "}
              <em style={{ color: "#c5d0df" }}>{city.name}</em>
            </h2>
            <p style={{ fontSize: 16, color: "rgba(255,255,255,0.55)", maxWidth: 520, lineHeight: 1.7, marginTop: 10 }}>
              Tell us about your {city.name} project and we&apos;ll respond within 2 business hours with a clear cost range.
            </p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 40, alignItems: "start" }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: 28 }}>
              <h3 style={{ fontFamily: "var(--font-display, Georgia, serif)", fontSize: 18, color: "white", marginBottom: 20 }}>Contact us directly</h3>
              {[
                ...
                <div key={c.label} style={{ paddingBottom: 16, marginBottom: 16, borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                  ...
                  {c.href
                    ? <a href={c.href} style={{ fontSize: 15, fontWeight: 600, color: "#c5d0df", textDecoration: "none" }}>{c.val}</a>
                    : <div style={{ fontSize: 15, color: "rgba(255,255,255,0.6)" }}>{c.val}</div>
                  }
                </div>
              ))}
              <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)", lineHeight: 1.6 }}>
                Every request is reviewed by a real team member. We respond within 2 business hours.
              </p>
            </div>
          </div>
      ...
      <section className="section" style={{ background: "var(--bg)" }}>
        <div className="container">
          <div style={{ marginBottom: 28 }}>
            ...
            <h2 style={{ fontFamily: "var(--font-display, Georgia, serif)", fontSize: "clamp(22px, 2.5vw, 32px)", color: "var(--navy)" }}>
              We Also Serve
            </h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
            {otherCities.map((c) => (
              <Link
                key={c.slug}
                href={`/cities/${c.slug}`}
                style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", background: "white", border: "1px solid var(--border)", borderRadius: 6, textDecoration: "none" }}
              >
                <div>
          ...
          <style>{`@media(max-width:700px){section .container>div:last-child{grid-template-columns:1fr!important}}`}</style>
        </div>
      </section>
    </>
  );
